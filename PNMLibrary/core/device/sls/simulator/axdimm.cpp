...
AxdimmSimulator::AxdimmSimulator(MemMap &mem_map)
    : psum_memory{std::make_unique<NBufferedMemory>(
          mem_map, psum_single_buf_size(), pnm::device::topo().NumOfPSumBuf,
          SLS_BLOCK_PSUM)},
      tags_memory{std::make_unique<NBufferedMemory>(
          mem_map, tags_single_buf_size(), pnm::device::topo().NumOfTagsBuf,
          SLS_BLOCK_TAGS)},
      min_psum_offsets_(topo().NumOfPSumBuf,
                        OffsetComputeUnitMap(topo().NumOfRanks)),
...
AxdimmSimulator::current_psum_buf(uint8_t compute_unit, uint32_t offset) const {
  return psum_memory->get_buffer(compute_unit, NBufferedMemory::Type::READ) +
         offset;
}

pnm::sls::device::InterleavedPointer
AxdimmSimulator::current_tags_buf(uint8_t compute_unit, uint32_t offset) const {
  return tags_memory->get_buffer(compute_unit, NBufferedMemory::Type::READ) +
         offset;
}

void AxdimmSimulator::handle_psum_buf_read(uint8_t compute_unit,
                                           uint32_t offset) {
  const auto buf_idx =
...
void AxdimmSimulator::handle_tags_buf_read(uint8_t compute_unit,
                                           uint32_t offset) {
  const auto buf_idx =
...
void AxdimmSimulator::write_reg(uint8_t *reg_addr, uint32_t val) {
  *reinterpret_cast<uint32_t *>(reg_addr) = val;
}

template <typename T>
void AxdimmSimulator::sparse_ft_sum(
...
void AxdimmSimulator::sparse_ft_sum_tagged(
    InterleavedPointer ft_table, InterleavedPointer psum,
    InterleavedPointer tags,
    uint64_t bytes_per_idx) NO_SANITIZE_UNSIGNED_INTEGER_OVERFLOW {
  sparse_ft_sum<uint32_t>(ft_table, psum, bytes_per_idx);
  *tags.as<pnm::uint128_t>() +=
      *(ft_table + bytes_per_idx).as<pnm::uint128_t>();
}

void AxdimmSimulator::exec_trace(uint8_t compute_unit) {
  ...
  const auto HA = rank_to_ha(compute_unit);
  const InterleavedPointer cfgr_base(
      mem_map_[compute_unit].addr[SLS_BLOCK_CFGR], HA);
  auto *polling_reg =
      (cfgr_base + topo().RegPolling + sizeof(uint32_t)).as<uint8_t>();
  ...
  /* set poll register */
  write_reg(polling_reg, POLL_FINISH_VALUE);
}

void AxdimmSimulator::clear_memory(pnm::sls::device::InterleavedPointer ptr,
                                   size_t size) {
...
void AxdimmSimulator::clear_buf_common(OffsetBufMapRef min_offsets,
                                       OffsetBufMapRef max_offsets,
                                       NBufferedMemory &memory,
                                       uint64_t buffer_size,
                                       uint8_t compute_unit, uint8_t buf_idx,
                                       uint32_t offset) {
...
void AxdimmSimulator::clear_psum_buf(uint8_t compute_unit, uint8_t buf_idx,
                                     uint32_t offset) {
...
void AxdimmSimulator::clear_tags_buf(uint8_t compute_unit, uint8_t buf_idx,
                                     uint32_t offset) {
