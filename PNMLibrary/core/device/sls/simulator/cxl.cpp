...
CXLSLSSimulator::CXLSLSSimulator(const MemMap &mem_map) : mem_map_{mem_map} {}

void CXLSLSSimulator::write_reg(uint8_t *reg_addr, uint32_t val) {
  *reinterpret_cast<volatile uint32_t *>(reg_addr) = val;
}

template <typename T>
void CXLSLSSimulator::sparse_ft_sum(const uint8_t *channel_base,
                                    uint64_t channel_addr, uint32_t output_idx,
                                    uint8_t *psum_buf, uint64_t bytes_per_idx) {
...
void CXLSLSSimulator::sparse_ft_sum_tagged(
  ...
  sparse_ft_sum<uint32_t>(channel_base, channel_addr, output_idx, psum_buf,
                          bytes_per_idx);
  const auto *const tag_table = reinterpret_cast<const pnm::uint128_t *>(
      channel_base + channel_addr + bytes_per_idx);

  auto *const tag_psum = reinterpret_cast<pnm::uint128_t *>(
      tags_buf + output_idx * topo().AlignedTagSize);

  *tag_psum += *tag_table;
}

void CXLSLSSimulator::exec_trace(uint8_t channel) {
  ...
  uint8_t *polling_reg = mem_map_[channel].addr[SLS_BLOCK_CFGR] +
                         topo().RegPolling + sizeof(uint32_t);
  ...
  const auto instructions =
      pnm::make_view(instr_begin, instr_begin + inst_per_single_buf_cached);
  ...
  for (uint32_t i = 0; i < inst_per_single_buf_cached; i++) {
    ...
    if (!received_header) {
      ...
      const auto channel_inst =
          sls::ChannelInstGenerator::decode_instruction(raw_instr);
      const uint64_t channel_addr =
          sls::ChannelInstGenerator::encode_address_from_instruction(
              channel_inst);

      const auto &[cs, instr] = channel_inst;
...
void CXLSLSSimulator::clear_buffers([[maybe_unused]] uint8_t channel) {
