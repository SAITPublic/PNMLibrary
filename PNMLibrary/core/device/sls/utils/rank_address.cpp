...
uint64_t memcpy_block(const uint8_t *&source, uint64_t block_size,
  ...
  if constexpr (PNM_PLATFORM != FUNCSIM) {
    pnm::memory::flush(dst.get(), block_size);
    pnm::memory::mfence();
  }
  dst += block_size;
  source += block_size;
...
InterleavedPointer make_interleaved_pointer(uint32_t cunit,
  ...
  const auto HA = cunit_to_ha(cunit);
  ...
  const auto *base = virtual_addr - interleaved_address(phys_offset, HA);

  return InterleavedPointer(base, HA) + phys_offset;
}
