...
NBufferedMemory::NBufferedMemory(MemMap &memory, size_t single_buffer_size,
    ...
    : m_map_(memory), single_buffer_size_{single_buffer_size},
      ...
      indices_(topo().NumOfCUnits, std::vector<uint8_t>(Type::MAX)),
...
InterleavedPointer NBufferedMemory::get_buffer_by_index(uint8_t cunit,
                                                        uint8_t index) const {
  return InterleavedPointer(m_map_[cunit].addr[mem_block_type_],
                            cunit_to_ha(cunit)) +
