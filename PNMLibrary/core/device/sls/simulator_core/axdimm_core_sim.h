...
#ifndef _SLS_AXDIMM_CORE_SIMULATOR_H_
#define _SLS_AXDIMM_CORE_SIMULATOR_H_

namespace pnm::sls::device {

class AxdimmSimulatorCore {
public:
  explicit AxdimmSimulatorCore(MemMap &mem_map);
  ...
  InterleavedPointer current_psum_buf(uint8_t compute_unit,
                                      uint32_t offset) const;
  InterleavedPointer current_tags_buf(uint8_t compute_unit,
                                      uint32_t offset) const;
...
private:
  ...
  void clear_memory(InterleavedPointer ptr, size_t size);
...
#endif // _SLS_AXDIMM_CORE_SIMULATOR_H_
