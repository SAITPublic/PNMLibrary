...
constexpr auto rank_to_ha(uint8_t rank) {
  // 0 (0b00), 1 (0b01) -- HA 0
  // 2 (0b10), 3 (0b11) -- HA 1
  return (rank >> 1) & 0b1;
}
...
ALWAYS_INLINE uintptr_t interleaved_address(uintptr_t rank_address,
                                            uint8_t HA) {
  static constexpr auto HA_XOR_BIT = 16ULL;
  static constexpr auto HA_MASK = 0b1ULL;

  const auto r16 = (rank_address >> HA_XOR_BIT) & HA_MASK;
  HA = r16 ^ HA;

  static constexpr auto LOW_BIT_SIZE = 7;
  static constexpr auto LOW_BIT_MASK = ((1ULL << LOW_BIT_SIZE) - 1);

  const auto low_bits = rank_address & LOW_BIT_MASK;
  const auto hi_bits = rank_address & (~LOW_BIT_MASK);

  static constexpr auto HA_BIT_POS = 7ULL;
...
/**@brief Transform pointer to InterleavedPointer for specified rank*/
