...
namespace pnm::sls::internal::cpu {

using namespace pnm::views;

template <typename T> struct Reduction {
  static void common(rowwise<const T> table,
                     views::common<const uint32_t> indices,
                     views::common<T> psum) {
  ...
  static void
  on_tag(rowwise<const T> tags, views::common<const uint32_t> indices,
         views::common<T> psum_tag) NO_SANITIZE_UNSIGNED_INTEGER_OVERFLOW {

    auto &psum_tag_value =
        *reinterpret_cast<pnm::types::uint128_t *>(psum_tag.begin());
    psum_tag_value = pnm::types::uint128_t{};

    for (auto index : indices) {
      const auto tag = *(tags.begin() + index);
      psum_tag_value +=
          *reinterpret_cast<const pnm::types::uint128_t *>(tag.begin());
    }
  }
  ...
  static void compute(const T *table_begin, const std::vector<uint32_t> &rows,
    ...
    auto psum_v =
        make_rowwise_view(psum, psum + psum_size, sparse_feature_size);

    auto cpsum_it = psum_v.begin();
    for (auto rows_count : rows) {
      auto table_v = make_rowwise_view(
          table_begin, table_begin + sparse_feature_size * rows_count,
          sparse_feature_size);

      for (auto batch_id = 0U; batch_id < minibatch_size; ++batch_id) {
        auto indices_v = make_view(indices_begin, indices_begin + *length);

        Reduction<T>::common(table_v, indices_v, *cpsum_it);
  ...
  compute_with_tag(const T *table_begin, const std::vector<uint32_t> &rows,
    ...
    static constexpr auto single_tag_size =
        sizeof(pnm::types::uint128_t) / sizeof(T);
    ...
    auto tag_psum_v = make_rowwise_view(
    ...
    for (auto rows_count : rows) {
      ...
      for (auto batch_id = 0U; batch_id < minibatch_size; ++batch_id) {
        ...
        Reduction<T>::common(table_v, indices_v, *cpsum_it);
        Reduction<T>::on_tag(tag_v, indices_v, *ctag_it);
