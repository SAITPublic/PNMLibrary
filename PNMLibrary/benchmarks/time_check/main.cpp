...
int main() {
  ...
  auto table_generator = TablesGeneratorFactory::default_factory().create(
      "float_tapp",
      std::to_string(idx_values) + " " + std::to_string(sparse_feature_size));

  std::vector<uint32_t> rows(num_tables, table_len);
  const TablesInfo tinfo(num_tables, sparse_feature_size, rows);
  ...
  for (auto [mini_batch_size, num_lookup] : test_batch_sizes_num_lookups) {
    ...
    for (int i = 0; i < knum_requests; ++i) {
      sls_op.set_run_params(mini_batch_size, pnm::make_view(info.lengths()),
                            pnm::make_const_view(indices[i]),
