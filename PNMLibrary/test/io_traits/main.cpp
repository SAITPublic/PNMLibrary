...
TEST_F(io_traits_fixture, TrivialMemoryWriter) {
  ...
  EXPECT_EQ(io_traits<TrivialMemoryWriter>::meta_size,
            sizeof(pnm::types::uint128_t));
  ...
  EXPECT_EQ(io_traits<TrivialMemoryWriter>::memory_size<int>(probe_cols, true),
            probe_cols * sizeof(int) + sizeof(pnm::types::uint128_t));
}
...
TEST_F(io_traits_fixture, MemoryWriter) {
  ...
  EXPECT_EQ(io_traits<MemoryWriter>::meta_size, topo().AlignedTagSize);
  ...
  EXPECT_EQ(io_traits<MemoryWriter>::memory_size<int>(probe_cols, true),
            probe_cols * sizeof(int) + topo().AlignedTagSize);
}
...
TEST_F(io_traits_fixture, UserWriter) {
  ...
  EXPECT_EQ(io_traits<UserWriter>::meta_size, sizeof(pnm::types::uint128_t));
